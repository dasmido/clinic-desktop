...
import { databaseQuery, type Appointment, type AppointmentStatus, type Patient } from '@/modules/clinic-data'
...
const form = ref({ patientId: '', startsAt: '', endsAt: '', status: 'scheduled' as AppointmentStatus, notes: '' })
...
function openCreateModal() {
  ...
  form.value = { patientId: '', startsAt: localDateTime(start), endsAt: localDateTime(end), status: 'scheduled', notes: '' }
  errorMessage.value = ''
...
function openEditModal(appointment: Appointment) {
  ...
  form.value = { patientId: String(appointment.patient_id), startsAt: localDateTime(new Date(appointment.starts_at)), endsAt: localDateTime(new Date(appointment.ends_at)), status: appointment.status, notes: appointment.notes }
  errorMessage.value = ''
  isModalOpen.value = true
}
...
async function loadSchedule() {
  ...
  try {
    ...
    appointments.value = await databaseQuery<Appointment>(`
      SELECT appointments.id, appointments.patient_id, patients.full_name AS patient_name, patients.phone AS patient_phone,
...
async function saveAppointment() {
  if (!form.value.patientId || !form.value.startsAt || !form.value.endsAt) {
    errorMessage.value = 'اختر المراجع ووقت بداية ونهاية الموعد.'
    return
  }
  ...
  try {
    const conflict = await databaseQuery<{ id: number }>(`
      SELECT id FROM appointments WHERE status != 'cancelled' AND starts_at < $2 AND ends_at > $1${editingAppointment.value ? ' AND id != $3' : ''} LIMIT 1
    `, editingAppointment.value ? [startsAt.toISOString(), endsAt.toISOString(), editingAppointment.value.id] : [startsAt.toISOString(), endsAt.toISOString()])
    if (form.value.status !== 'cancelled' && conflict.length) {
      errorMessage.value = 'يوجد موعد نشط خلال هذا الوقت.'
      return
    }
    const values = [Number(form.value.patientId), startsAt.toISOString(), endsAt.toISOString(), form.value.status, form.value.notes.trim()]
    if (editingAppointment.value) {
      await databaseQuery('UPDATE appointments SET patient_id = $1, starts_at = $2, ends_at = $3, status = $4, notes = $5, updated_at = now() WHERE id = $6', [...values, editingAppointment.value.id])
    } else {
      await databaseQuery('INSERT INTO appointments (patient_id, starts_at, ends_at, status, notes) VALUES ($1, $2, $3, $4, $5)', values)
    }
...
onMounted(async () => { await Promise.all([loadSchedule(), loadPatients()]) })
</script>
...
<template>
  <section class="mx-auto w-full max-w-6xl space-y-6">
    <header class="flex flex-col gap-4 border-b border-default pb-5 sm:flex-row sm:items-end sm:justify-between">
      ...
      <UButton icon="i-lucide-calendar-plus" label="موعد جديد" :disabled="!patients.length" @click="openCreateModal" />
    </header>
    ...
    <div v-if="!patients.length && !isLoading" class="border border-dashed border-default p-10 text-center"><p class="font-semibold text-highlighted">أضف مراجعاً أولاً</p><p class="mt-1 text-sm text-muted">لا يمكن إنشاء موعد دون سجل مراجع.</p><UButton to="/patients" class="mt-4" label="الذهاب إلى المراجعين" /></div>
    <div v-else class="overflow-hidden border border-default bg-default shadow-sm">
      ...
      <div v-else class="divide-y divide-default">
        <button v-for="appointment in visibleAppointments" :key="appointment.id" class="grid w-full grid-cols-[5.75rem_1fr_auto] items-center gap-4 px-5 py-4 text-right transition-colors hover:bg-elevated/35" @click="openEditModal(appointment)">
          ...
          <span><span class="block font-semibold text-highlighted">{{ appointment.patient_name }}</span><span class="mt-0.5 block text-xs text-muted" dir="ltr">{{ appointment.patient_phone }}</span></span>
