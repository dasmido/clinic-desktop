...
function goBack() {
  void router.push(`/patients/${patientId}/records`);
}
...
async function saveRecord() {
  ...
  try {
    await window.electronAPI.medicalRecords.create({
      ...
      temperature_celsius: form.value.temperature
        ? Number(form.value.temperature)
        : null,
    ...
    errorMessage.value =
      error instanceof Error ? error.message : "تعذر حفظ السجل الطبي.";
  } finally {
...
onMounted(async () => {
  try {
    const [patients, doctorRows] = await Promise.all([
      window.electronAPI.patients.list(),
      window.electronAPI.doctors.list(),
    ]);
    patient.value = patients.find((item) => item.id === patientId) ?? null;
    doctors.value = doctorRows;
    form.value.visitDate = localDateTime();
  } catch (error) {
    errorMessage.value =
      error instanceof Error ? error.message : "تعذر تحميل بيانات المراجع.";
  }
});
</script>
<template>
  <section class="mx-auto w-full max-w-5xl space-y-6">
    <header class="border-b border-default pb-5">
      <UButton
        ...
        color="neutral"
        variant="ghost"
        :label="patient?.full_name ?? 'السجل الطبي'"
        class="-me-2 mb-3"
        @click="goBack"
      />
      <h1 class="text-2xl font-bold text-highlighted">تسجيل زيارة طبية</h1>
      <p v-if="patient" class="mt-2 text-sm text-muted">
        {{ patient.full_name }} · <span dir="ltr">{{ patient.phone }}</span>
      </p>
    </header>
    <form
      class="space-y-5 border border-default bg-default p-5 shadow-sm sm:p-7"
    ...
    >
      <UFormField label="تاريخ ووقت الزيارة" required
        ><UInput
          ...
          type="datetime-local"
          class="w-full"
          autofocus /></UFormField
      ><UFormField label="الطبيب"
        ><USelect
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        ><UFormField label="التشخيص"
          ><UTextarea v-model="form.diagnosis" class="w-full" :rows="3"
        /></UFormField>
      </div>
      <UFormField label="الخطة العلاجية"
        ><UTextarea v-model="form.treatmentPlan" class="w-full" :rows="3"
      /></UFormField>
      <div class="grid gap-4 sm:grid-cols-3">
        <UFormField label="ضغط الدم"
          ><UInput
            ...
            class="w-full"
            dir="ltr" /></UFormField
        ><UFormField label="الحرارة (°م)"
          ><UInput
            v-model="form.temperature"
            type="number"
            step="0.1"
            class="w-full" /></UFormField
        ><UFormField label="الوزن (كغ)"
          ><UInput
            v-model="form.weight"
            type="number"
            step="0.1"
      ...
      <UFormField label="ملاحظات سريرية"
        ><UTextarea v-model="form.clinicalNotes" class="w-full" :rows="3"
      /></UFormField>
      ...
      <div class="flex justify-end gap-2 border-t border-default pt-5">
        <UButton
          ...
          label="إلغاء"
          @click="goBack"
        /><UButton type="submit" :loading="isSaving" label="حفظ الزيارة" />
      </div>
    </form>
  </section>
</template>
