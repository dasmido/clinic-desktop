...
<template>
  <section class="flex h-screen items-center justify-center p-4">
    <UCard class="w-full max-w-sm">
      <template #header>
        <h1 class="text-xl font-bold">{{ t('auth.registerTitle') }}</h1>
        <p class="text-muted mt-1">{{ t('auth.registerSubtitle') }}</p>
      </template>
      ...
      <form class="space-y-5" @submit.prevent="onSubmit">
        ...
        <UButton type="submit" block :loading="loading" :label="t('auth.register')" />
      </form>
    </UCard>
  </section>
</template>